...
export default function HeroSection() {
    return (
        <div className='flex flex-col gap-y-7'>
            <span className='block font-gilmer font text-center text-pretty text-xl sm:text-2xl md:text-4xl xl:text-5xl 3xl:text-6xl font-bold tracking-wide lg:tracking-wider p-1'>Çeşitli Kategorilerde Trend Ürünler</span>
                ...
                {productDetails?.map((product) => (
                    <div className='w-full h-[350px] md:h-[400px] lg:h-[500px] xl:h-[550px] 2xl:h-[600px] p-0 lg:p-1 gap-y-1 flex flex-col justify-start lg:items-start bg-[#fff] rounded-2xl shadow-md border transition-all ' key={product.id}>
                        <div className='w-full h-72 md:h-96 xl:h-[400px] 2xl:h-[500px] overflow-hidden rounded-t-xl rounded-b-md cursor-pointer shadow-md transition-all hover:shadow-xl'>
                            <img loading='lazy' className='w-full h-full object-cover shadow-2xl transition-all duration-500 hover:scale-150' src={product.image} alt="suggestedProductImage" />
                        </div>
